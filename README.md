...
python qr_reader.py --stdout
```

Use `--stdout` to print the decoded QR content without creating a file. `--stdout`
and `--output` are mutually exclusive.

### Show a static QR code from standard input

Pipe text into `qr_show_stdin.py`, or run it interactively and press `Ctrl+D` when
the input is complete. Press `q` or Escape to close the QR window.

```bash
printf '%s' 'Text to encode' | python qr_show_stdin.py
python qr_show_stdin.py
```

### Show a static QR code from a file

`qr_show_file.py` encodes the file's raw byte stream. A single QR using the
project's settings can hold at most 2,953 file bytes; larger files produce an
error instead of opening a window. Press `q` or Escape to close the QR window.

```bash
python qr_show_file.py /path/to/file
```
